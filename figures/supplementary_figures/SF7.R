...
# Linear models with accessibility log2FoldChange at promoters
# (identical in construction to the expression ones)
  library(dplyr)
  library(tidyr)
  library(broom)
...
lm_fits <- lapply(split(df_prom, df_prom$study), function(df_study) {
    ...
    # 2. CRITICAL CLEANUP: Keep only real and finite numerical values
    df_study_clean <- df_study %>% 
        dplyr::filter(is.finite(log2FoldChange))
    
    # 3. Predictor variance validation
  ...
  # LM on promoters controlling for initial accessibility
  ...
  # Compact version of the heatmap for promoters (smaller PDF and dynamic according to number of studies)
  plot_model_heatmap_small <- function(df, title, outfile) {
    ...
    # Dynamic size: shorter height if there are fewer studies
    n_rows <- length(rows)
    n_cols <- length(cols)
    ...
    # Search for and reduce title font size robustly
    ...
    if (length(title_idx) > 0) {
      ...
      if (!is.null(grob$gp) && !is.null(grob$gp$fontsize)) {
        ...
        # Fallback: try assigning gp directly
        grob$gp <- grid::gpar(fontsize = title_fontsize)
        }
      }
...
lm_fits <- lapply(split(df_expr_acc, df_expr_acc$study), function(df_study) {
    ...
    # 5. CONSTRUCT FORMULA WITH BACKTICKS (The key correction)
...
human <- useEnsembl(
  ...
  # optionally: mirror = "useast", or version = 110, etc.
)

orthologs <- getBM(
  attributes = c(
    "ensembl_gene_id",
    "external_gene_name",                      # generic human symbol
    "mmusculus_homolog_ensembl_gene",         # mouse ortholog Ensembl ID
    "mmusculus_homolog_associated_gene_name"  # mouse symbol (MGI)
  ),
  filters = "hgnc_symbol",                    # we use HGNC ONLY as a filter
